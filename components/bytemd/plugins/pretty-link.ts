...
// 超链接通用 svg icon node
const linkSvgNode = createElementFromHtml(
...
const bilibiliSvgNode = createElementFromHtml(
...
// 掘金 svg icon node
const juejinSvgNode = createElementFromHtml(
...
// 某国外视频网站
const ytSvgNode = createElementFromHtml(
...
const githubSvgNode = createElementFromHtml(
...
const stackoverflowSvgNode = createElementFromHtml(
...
const getExternalDomain = (href: string) => {
  try {
    ...
    return extractDomainFromUrl(href);
  } catch {
    return null;
  }
};
      ...
      process.use(() => (tree: Root) => {
        visit(tree, "element", (node: Element) => {
          if (node.tagName === "a") {
            ...
            const host = getExternalDomain(href);
            if (!host) {
              return;
            }
            ...
            node.children.push(cloneHastNode(linkSvgNode));

            // 判断当前链接是哪种，是否为已知的网站，根据域名添加对应图标
            const svgNode = domainToSvgNodeMap[host];
            if (svgNode) {
              node.children.unshift(cloneHastNode(svgNode));
            }
          }
        });
      }),
